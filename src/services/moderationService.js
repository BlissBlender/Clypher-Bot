...
export class ModerationService {
  ...
  static assertModerationHierarchy(moderator, target, action) {
    ...
    if (!botCheck.valid) {
      throw new ClypherBotError(botCheck.error, ErrorTypes.PERMISSION, botCheck.error);
    }

    const modCheck = this.validateHierarchy(moderator, target, action);
    if (!modCheck.valid) {
      throw new ClypherBotError(modCheck.error, ErrorTypes.PERMISSION, modCheck.error);
    }
  }
